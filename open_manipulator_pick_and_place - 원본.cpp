...
{ // 조인트 관절 경로 설정 - 특정 조인트 이름과 각도를 사용해 경로 설정
...
{ // 로봇의 이동 중인지 상태를 확인하여 이동 중 여부를 업데이트
...
{ // 조인트 상태 데이터를 받아 현재 조인트 각도를 업데이트
...
{
  if (ch == '1')
    mode_state_ = HOME_POSE; // 초기 자세
  else if (ch == '2')
  {
    mode_state_ = DEMO_START; // 데모 시작
    demo_count_ = 0;
  }
  else if (ch == '3')
    mode_state_ = DEMO_STOP; // 데모 정지
}

void OpenManipulatorPickandPlace::demoSequence()
{
  std::vector<double> joint_angle; // 조인트 각도 설정
  std::vector<double> kinematics_position; // 그리퍼의 3D 위치
  std::vector<double> kinematics_orientation; // 그리퍼의 방향(회전 값)
  std::vector<double> gripper_value; // 그리퍼 열림/닫힘 정도
